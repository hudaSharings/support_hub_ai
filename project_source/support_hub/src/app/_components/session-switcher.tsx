  ...
  return (
    <div className="flex w-full items-center justify-between gap-3">
      <nav className="flex items-center gap-2 text-sm">
        <Link href="/" className="rounded-md border px-2 py-1 text-xs font-medium hover:bg-accent">
          Dashboard
        </Link>
        {session.role === "admin" || session.isSuperAdmin ? (
          <Link href="/admin" className="rounded-md border px-2 py-1 text-xs font-medium hover:bg-accent">
            Admin
          </Link>
        ) : null}
      </nav>
