...
## 8. Use Case and Scenario Flows

### 8.1 Use Case Overview

```mermaid
flowchart LR
    A[Support Analyst submits case] --> B{Case category}
    B -->|Billing/Plan| C[Check subscription + invoice + entitlement]
    B -->|Token/Auth| D[Check PAT status + org policy + SSO auth]
    B -->|SAML/Identity| E[Check SAML config + incidents + history]
    C --> F[Decision Engine]
    D --> F
    E --> F
    F --> G[Resolve]
...
### 8.2 Scenario Flow: PAT failing for organization resources
...
sequenceDiagram
    ...
    participant TL as Tool Agent
    participant RG as RAG Agent
    participant DC as Decision Agent

    SA->>WF: "PAT works for user repos, fails on org resources"
    WF->>TL: run token diagnostics + org policy check
    TL-->>WF: token permissions + SSO authorization + policy findings
    WF->>RG: retrieve PAT + SAML docs
    RG-->>WF: cited docs for PAT scope and SSO authorization
    WF->>DC: combine tool and doc evidence
    DC-->>WF: resolve/clarify/escalate with rationale
```

### 8.3 Scenario Flow: API rate-limit complaint
...
sequenceDiagram
    ...
    SA->>WF: "GitHub API is failing for our org"
    WF->>TL: check API usage + throttled requests + incident status
    TL-->>WF: high request count, throttled responses, no major outage
    WF->>RG: fetch rate-limit and troubleshooting docs
    RG-->>WF: citations explaining limits and mitigation
    WF->>RS: generate grounded customer response + internal note
    RS-->>SA: explain rate limit cause and next actions
```

---

## 9. Decision Policy (Business Logic at High Level)
